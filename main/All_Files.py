...
for address, hex_value in zip(LnT_DO_Address_List, swapped_hex_values):
    command = f"sshpass -p 'test123' ssh test@192.168.2.100 /usr/share/vmetoolkit/examples/singleAccess/MasterSgl w 0x39 {address} 16 {hex_value}"
...
LnT_AO_Address_List = [ '0x970C02', '0x970C04', '0x970C06', '0x970C08', '0x970C0A', '0x970C0C',
                        '0x970C0E', '0x970C10']
...
for address, voltage in zip(LnT_AO_Address_List, LnT_AO_Address_Values):
    ...
    if hex_output:
        ...
        command = f"sshpass -p 'test123' ssh test@192.168.2.100 /usr/share/vmetoolkit/examples/singleAccess/MasterSgl W 0x39 {address} 16 {shifted_output}"
...
def Ref_DI_AI_Writedata(portname):
    try:
        with serial.Serial(portname, baudrate=115200, timeout=1) as ser:
            # DI_Ref_Binary = "1" * 208
            DI_Ref_Binary = "1000000000000001100000000000000111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111"
               
            # DI_Ref_Binary = "0000100111111111111111111111111100111011111011111000000000000000000000000111111100010101111111100000000000000000000000000000000010000000000000000000000000000000000000000000000000010000000000000000000001010000"
            ...
            message = DI_Ref_Value_Hex + "00360" * 48
