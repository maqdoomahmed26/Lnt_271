...
def ret_btn_result():
    ...
    with serial.Serial(portname, baudrate=115200, timeout=1) as ser:
        while True:
            ...
            if ser.in_waiting:
                Ref_AO_N10V_Result = ser.readline().decode('utf-8').strip()
                print(f"Received: {Ref_AO_N10V_Result}")
                break
    ...
    def parse_voltage_string(data):
        ...
        return voltages
    result = parse_voltage_string(Ref_AO_N10V_Result)
    return result

#Getting RESULT   (Display in Ref AO -10V Column)

#   Ref_AO_N10V_Result = FFFFFFFFFFFFFFFFFFFFFFFF110001100011000110001100011000110001100011000110001100011000

#   Separate digital value and analog value from Ref_AO_N10V_Result
#  Ref_AO_N10V_Result = FFFFFFFFFFFFFFFFFFFFFFFF
# 11000 11000 11000 11000 11000 11000 11000 11000 11000 11000 11000 11000

#RESULT   (Display in Ref AO -10V Column)

# Ref_AO_N10V_Result = [-10.00, -10.00, -10.00, -10.00, -10.00, -10.00, 
#                     -10.00, -10.00, -10.00, -10.00, -10.00, -10.00]
