...
def ret_btn_result():
    ...
    with serial.Serial(portname, baudrate=115200, timeout=1) as ser:
        while True:
            ...
            if ser.in_waiting:
                AO_0V_Result = ser.readline().decode('utf-8').strip()
                print(f"Received: {AO_0V_Result}")
                break
    ...
    def parse_voltage_string(data):
        ...
        return voltages
    result = parse_voltage_string(AO_0V_Result)
    return result

#Getting RESULT   (Display in Ref AO -10V Column)

#   AO_0V_Result = FFFFFFFFFFFFFFFFFFFFFFFF110001100011000110001100011000110001100011000110001100011000

#   Separate digital value and analog value from AO_0V_Result
#  AO_0V_Result = FFFFFFFFFFFFFFFFFFFFFFFF
# 11000 11000 11000 11000 11000 11000 11000 11000 11000 11000 11000 11000

#RESULT   (Display in Ref AO -10V Column)

# AO_0V_Result = [+00.00, +00.00, +00.00, +00.00, +00.00, +00.00, 
#                     +00.00, +00.00, +00.00, +00.00, +00.00, +00.00]
